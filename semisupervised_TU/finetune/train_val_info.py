...
import sys
sys.path.append('/home/ailaw/wangshuo/GraphCL/semisupervised_TU')
sys.path.append('/home/ailaw/wangshuo/GraphCL/semisupervised_TU/InfoGraph/unsupervised')
from InfoGraph.unsupervised.main import InfoGraph,evaluate_embedding


device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')


def cross_validation_with_val_set_info(dataset,
                                  model_func,
                                  epochs,
                                  batch_size,
                                  lr,
    ...
    for fold, (train_idx, test_idx, val_idx) in enumerate(
        ...
        model = InfoGraph(dataset,128, 3).to(device)
        # # train from scratch
        pretrained_dict = torch.load(model_PATH)#只加载预训练中相同的参数
        model_dict = model.state_dict()

        keys = []
        for k, v in pretrained_dict.items():  # 只加载预训练中相同的参数
            keys.append(k)
        i = 0
...
def train(model, optimizer, loader, device):
    ...
    for data in loader:
        ...
        out = model.encoder.get_embeddings(data)
...
def eval_acc(model, loader, device, with_eval_mode):
    ...
    for data in loader:
        data = data.to(device)
        with torch.no_grad():
            pred = model.encoder.get_embeddings(data).max(1)[1]
        correct += pred.eq(data.y.view(-1)).sum().item()
