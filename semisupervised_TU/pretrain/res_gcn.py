...
class ResGCN(torch.nn.Module):
    ...
    def __init__(self, dataset=None, hidden=128, num_feat_layers=1, num_conv_layers=3,
        ...
        hidden_in = dataset.num_features#直接换成128不知道会怎么样？
        if collapse:
...
class ResGCN_graphcl(ResGCN):
    ...
    def loss_graphcl(self, x, x1, x2, mean=True, Loss=3, beta=0.3):#Loss 取 1,2,3
        T = 0.5
        ...
        sim_matrix_1 = sim_matrix_1.masked_fill(sim_matrix_1 == s_m_max.expand(sim_matrix_1.size()), 0)#不加掩码
        sim_matrix_2 = sim_matrix_2.masked_fill(sim_matrix_2 == s_m_max.expand(sim_matrix_2.size()), 0)

        # sim_matrix_1 = sim_matrix_1.masked_fill(sim_matrix_1 > s_m_0.expand(sim_matrix_1.size()), 0)#增加掩码
        # sim_matrix_2 = sim_matrix_2.masked_fill(sim_matrix_2 > s_m_0.expand(sim_matrix_2.size()), 0)
        ...
        if Loss==1:
            ...
            loss = s_m_0.T / (sim_matrix_1.sum(dim=1) + sim_matrix_2.sum(dim=1) - s_m_0)
            loss = - torch.log(loss)
            if mean:
                loss = loss.mean()
            return loss + 1 / pos_sim.mean(), [MinSLB, Sub, DSum]
        else :
            s_m_max = s_m_max.masked_fill(pos_sim <= s_m_0.mean(), 1e-5)  # 全程拉近min，当相似时拉近max，不加尾项
        ...
        # print("1")
        # print((sim_matrix_1.sum(dim=1) + sim_matrix_2.sum(dim=1)).mean())
        # print((s_m_max).mean())
        # print((s_m_max-s_m_0).mean())
