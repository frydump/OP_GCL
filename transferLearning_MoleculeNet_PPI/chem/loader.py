...
class MoleculeDataset_graphcl_graphaf(MoleculeDataset_graphcl):
    def __init__(self,
                 root,
                 transform=None,
                 pre_transform=None,
                 ...
                 empty=False):
        self.set_idxs([1,2,3])
        self.set_nl_el(None, None)
        self.idxs_all = np.load('./GraphAF/zinc_pregnn/idxs_valid.npy').tolist()
        super(MoleculeDataset_graphcl_graphaf, self).__init__(root, transform, pre_transform, pre_filter, dataset, empty)

    def generator_generate(self, nl, el, n_generator=1):
        with torch.no_grad():
            mol = self.generators[n_generator-1].generate_one_molecule_with_latent_provided(nl, el)
    ...
    def set_nl_el(self, nl_el_1, nl_el_2):
        self.nl_el_1, self.nl_el_2 = nl_el_1, nl_el_2

    def __len__(self):
        return len(self.idxs)

    def get(self, idx_batch):
        idx = self.idxs_all[self.idxs[idx_batch]]
        data, data1, data2 = Data(), Data(), Data()
        for key in self.data.keys:
            item, slices = self.data[key], self.slices[key]
            s = list(repeat(slice(None), item.dim()))
        ...
        data1 = self.generator_generate(self.nl_el_1[0][idx_batch], self.nl_el_1[1][idx_batch], 1)
        # data2 = self.generator_generate(self.nl_el_1[0][idx_batch], self.nl_el_1[1][idx_batch], 2)
...
if __name__ == "__main__":
    # data = load_zinc_dataset('./dataset/zinc')
    create_all_datasets()
