...
class BatchMasking(Data):
    ...
    def from_data_list(data_list):
        ...
        for key in keys:
            dims = data_list[0].cat_dim(key, batch[key][0])
            batch[key] = [i for i in batch[key]]
            if type(batch[key][0])==int:
                batch[key]=torch.tensor(batch[key])
            else:
                batch[key] = torch.cat(batch[key], dims)
        batch.batch = torch.cat(batch.batch, -1)
        return batch.contiguous()
